...
def Check(array, start, end, steps):
    # print(steps)
    ...
    if steps == 0:
        ...
        if (sum1*0.9 > sum2) & (sum1*0.9 > sum3):
            print("Up", sum1, sum2, sum3)
            return Check(array, start, half, steps)
        elif (sum3*0.9 > sum1) & (sum3*0.9 > sum2):
            print("Down", sum1, sum2, sum3)
            return Check(array, half, end, steps)
        else:
            print("stay", sum1, sum2, sum3)
            return Check(array, quarter, three_quarters, steps)
